...
func (variable Variable) GetValue(assignmentMap AssignmentMap) uint64 {
	if assignment, has := assignmentMap[string(variable)]; has {
...
func (index *Index) incrementIndex(position uint8) {
	if position == 0 {
		index.Subject++
	} else if position == 1 {
		index.Predicate++
...
func (indexMap IndexMap) GetIndex(node ld.Node, txn *badger.Txn) (*Index, error) {
	...
	indexKey := make([]byte, 2, 2+len(value))
...
func assembleKey(prefix byte, a, b, c uint64) []byte {
	...
	A := make([]byte, 8)
	binary.BigEndian.PutUint64(A, a)
	key = append(key, A...)
	...
	B := make([]byte, 8)
	binary.BigEndian.PutUint64(B, b)
	key = append(key, B...)
	...
	C := make([]byte, 8)
	binary.BigEndian.PutUint64(C, c)
	key = append(key, C...)
